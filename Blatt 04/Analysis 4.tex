...
Wer bemerken zunächst, dass $\pi(B)^c \subseteq \pi(B^c)$: Für $y \in \pi(B)^c$ ist $y \not\in \pi(B)$. Aus der Surjektivität von $\pi$ folgt, dass es $x \in \R^2$ mit $\pi(x) = y$ gibt. Da $y \not\in \pi(B)$ ist $x \not\in B$. Also ist $x \in B^c$ und daher $y \in \pi(B^c)$.
...
\begin{align*}
 ...
 \underset{\lambda_1^* \text{ monoton}}{\leq} \lambda_1^*(A \cap \pi(B)) + \lambda_1^*(A \cap \pi(B^c)) \\
 ...
 = \lambda_1^*(A).
\end{align*}
Dies zeigt, dass $\pi(B)$ $\lambda_1^*$-messbar ist.
