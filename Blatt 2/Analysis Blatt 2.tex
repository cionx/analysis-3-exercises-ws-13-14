...
\subsection{}
Sei $U \subseteq \R^n$ offen beliebig aber fest. Es gilt zu zeigen, dass $U \in F_\sigma$, d.h. dass es eine abzählbar unendliche Familie $(K_n)_{n \in \N}$ gibt, so dass $U = \bigcup_{n \in \N} K_n$.
...
Da nun
\[
 \{ B_{r(x)}(q(x)) : x \in U \} \subseteq \{B_r(q) : r \in \Q, r > 0, q \in \Q^n\}
\]
abzählbar ist, gibt es eine Folge $(x_n)_{n \in \N}$ so dass
\[
...
Für paarweise dijunkte $A_1, \ldots, A_N \in \A$ ist
\begin{align*}
