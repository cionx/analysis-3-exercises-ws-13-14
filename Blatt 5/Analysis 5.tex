...
Aus der Definition des Lebesgue-Maßes folgt, dass es eine Folge $(I_k)_{k \in \N}$ von $n$-dimensionalen offenen Intervallen gibt, so dass $N \subseteq \bigcup_{k \in \N} I_k$ und
\[
 \lambda_n(N)
 \leq \sum_{k \in \N} \Vol(I_k)
 = \sum_{k \in \N} \lambda_n(I_k)
 < \varepsilon.
\]

Da jedes der $I_k$ offen ist, gibt es, wie aus der Vorlesung bekannt, für alle $k \in \N$ eine disjunkte Familie halboffener Würfel $(C^k_l)_{l \in \N}$ mit $I_k = \bigdotcup_{l \in \N} C^k_l$ für alle $k$. (Im Skript ist dies \textbf{Lemma 1.32}; auch wenn dort im Satz nur von halboffenen Quadern gesprochen wird, handelt es sich bei den genutzten Quadern offenbar um Würfel.)
...
\begin{equation}\label{eq: volumenänderung würfel}
 \lambda_n(T(Q))
 = \prod_{i=1}^n \left( |\alpha_i| l \right)
...
\begin{align*}
 ...
 \leq \sum_{k \in \N} \lambda_n(T(Q_k)) \\
...
\subsection{}
Man bemerke, dass $\R^n \setminus \{0\} \in \mc{B}(\R^n)$ bezüglich $\mu$ eine Nullmenge ist. Da es für jedes $A \subseteq \R^n$ ein $B \subseteq \R^n \setminus \{0\}$ mit $A = \emptyset \cup B$ oder $A = \{0\} \cup B$ gibt, ist $\A_0 = \mc{P}(\R^n)$, denn $\emptyset, \{0\} \in \mc{B}(\R^n)$.
...
\end{align*}
$\tilde{\A}_0$ enthält genau die Teilmengen von $\R^2$, die $0 \times \R$ ganz oder gar nicht beinhalten. Es gilt $\tilde{\A}_0 = \A_0$:
...
\subsection{}
Sei $t \in \R$ beliebig aber fest. Da $N$ abzählbar ist, ist $N \in \mc{B}(\R^n)$. Es ist also auch $M := \R^n \setminus N \in \mc{B}(\R^n)$. Nach Definition von $N$ ist $f_{|M} : M \rightarrow \R^n$ stetig. Es ist daher
\[
...
\[
 \{x \in \R^n : f(x) \leq t\} = A \cup N' \in \mc{B}(\R^n),
\]
also $f$ Borell-messbar.


\subsection{}
Sei $t \in \R$ beliebig aber fest, und $A := \{x \in \R: f(x) < t\}$. Ist $A = \R$, so ist $A$ Borell-messbar. Ansonsten ist $A$ ein Intervall der Form $(-\infty,a)$ oder $(-\infty,a]$ mit $a \in \R$. Auch in diesen Fällen ist $A$ Borell-messbar.
...
\]
Borell- und damit auch Lebesgue-messbar. Für
\[
 N' := \{x \in N : f(x) \leq t\}
\]
...
\subsection{}
Sei $A \in \mc{B}(\R^n)$ und $\varepsilon > 0$ beliebig aber fest. 
...
\subsubsection*{(iv) $\Rightarrow$ (ii)}
...
\]
Da $C^c \supseteq A$ offen ist, und $\varepsilon > 0$ beliebig, zeigt dies (ii).
...
\subsection{}
\emph{Korrektur: Hier ist fast alles Blödsinn, und ich habe Lust die richtige Lösung abzutippen.}
Es sei
\[
