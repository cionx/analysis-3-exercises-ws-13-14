...
\[
 ...
 = 2.
\]
Also ist $f$ auf $(0,1)$ auch Lebesgue-integrierbar, weshalb $f \in \La^1((0,1))$. Es ist jedoch $f \not\in \La^2((0,1))$: Für alle $x \in (0,1)$ ist $|f(x)^2| = f(x)^2 = 1/x$. Für die Funktionenfolge
\[
 h_n := \chi_{[1/n,1)} f^2
\]
...
\subsection{}
Angenommen, es ist $f_n \longrightarrow f$ im Maß. Dann ist offenbar auch $f_{n_j} \longrightarrow f$ im Maß für jede Teilfolge $n_j$. Daher gibt es für jede Teilfolge $n_j$ eine Teilfolge $j_k$ so dass $f_{n_{j_k}}(x) \longrightarrow f(x)$ punktweise für $\mu$-fast alle $x \in \Omega$.

Die zu zeigende Umkehrung gilt im Allgemeinen nicht: Man betrachte etwa den Maßraum $(\R, \mc{B}(\R),\lambda)$ und die Funktionenfolge $f_n = \chi_{[n,\infty)}$ auf $\R$. Es ist $f_n \longrightarrow 0$ punktweise, also ist auch $f_{n_{j_k}} \longrightarrow 0$ punktweise für alle Teilfolgen $n_j$ und $j_k$. Es ist jedoch nicht $f_n \longrightarrow f$ im Maß. Die Implikation gilt jedoch für endliche Maßräume, weshalb wir die Implikation unter der zusätzlichen Annahme $\mu(\Omega) < \infty$ zeigen.
...
\[
 \limes{l}{\infty} \mu\left(\left\{x \in \Omega: \left|f_{n_{j_{k_l}}}(x) - f(x)\right| \geq \delta \right\}\right) = 0,
\]
...
\]
Es ist daher $f \in \La^\infty(\Omega,\A,\mu)$. (\emph{Bemerkung: Statt mit $M+1$ kann man auch mit $M+\varepsilon$ arbeiten, und erhält so sogar $\|f\|_\infty \leq M$.})
...
Aus diesen Annahmen folgt, dass $\|f\|_\infty > 0$. Sei $\|f\|_\infty \geq \varepsilon > 0$ beliebig aber fest, und
\[
...
\]
Aus der Beliebigkeit von $0 < \varepsilon \leq \|f\|_\infty$ folgt damit, dass
\[
...
\section{(Die Ordnung der $\La^p$-Räume)}
...
\begin{proof}
 ...
 \[
  ...
  < \infty,
 \]
 also $|f|^\alpha \in \La^{p/\alpha}(\Omega, \A, \mu)$, und es ist
 \[
  \| |f|^\alpha \|_{p/\alpha}
...
\]
Dabei ist $p/(r(1-\theta)), q/(r\theta) > 1$, da $(1-\theta)/p, \theta/q < 1$.
Da
\[
...
\[
 \|f\|_r
 = \left( \int_\Omega |f|^r \dmu \right)^{1/r}
