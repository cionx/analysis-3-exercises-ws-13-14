...
\section{(Dualität)}
Für den Fall, dass $f \in \cL^p(\Omega)$ ist die Aussage bereits aus der Vorlesung bekannt. Es ist nämlich bekannt, dass die Abbildung
\[
...
Sei nun $f : \Omega \to [-\infty, \infty]$ messbar und $p \in [0,\infty]$ mit $f \not\in \cL^p(\Omega)$. Wir unterscheiden zwischen den drei Fällen $p = 1$, $p = \infty$ und $1 < p < \infty$.

Ist $p = 1$, so lässt sich für alle $M \in \N$ die Testfunktion $g_M = \sgn(f)$ wählen. Es ist klar, dass $g_M \in \cL^\infty(\Omega)$ mit $\|g_M\|_\infty = 1$, da $f \not\in \cL^1(\Omega)$ ist
\[
...
Ist $p = \infty$ so setzen wir für beliebiges aber festes $M \in \N$
\[
 A := \{x \in \Omega : |f(x)| \geq M\}.
\]
Offenbar ist $A \in \A$, und da $f \not\in \cL^\infty(\Omega)$ ist $\mu(A) > 0$. Wegen der $\sigma$-Endlichkeit des Maßraums $(\Omega, \A, \mu)$ gibt es eine Menge $B \subseteq A$ mit $0 < \mu(B) < \infty$. Für die Funktion
\[
 g_M := \frac{\chi_B}{\mu(B)} \sgn(f)
\]
ist $g_M \in \cL^1(\Omega)$ mit $\|g_M\|_1 = 1$, da
\[
 \int_\Omega |g_M| \dmu
 = \frac{1}{\mu(B)} \int_B 1 \dmu
 = 1.
\]
Es ist
\[
 \int_\Omega f g \dmu
 = \frac{1}{\mu(B)} \int_B |f| \dmu
 \geq \frac{1}{\mu(B)} \int_B M \dmu
 = M.
\]
...
Da $|f| : \Omega \to [0,\infty]$ messbar ist gibt es eine Folge $(f_k)_{k \in \N}$ einfacher, messbarer Funktionen mit $f_k \geq 0$, $f_k \leq f_{k+1}$ für alle $k \in \N$ und $\limes{k}{\infty} f_k = |f|$ punktweise. Aufgrund von Monotonie und Stetigkeit ist auch $(f_k^p)_{k \in N}$ eine Folge einfacher, messbarer Funktionen mit $f_k^p \geq 0$, $f_k^p \leq f_{k+1}^p$ für alle $k \in \N$ und $\limes{k}{\infty} f_k^p = |f|^p$.
...
\[
 h := \chi_{B_N} f_K
\]
und bemerken direkt, dass $|f| \geq h \geq 0$.


Da $f_K^p$ als einfache Funktion beschränkt ist, und $\mu(B_N) < \infty$, ist
\[
 \int_\Omega |h|^p \dmu
 = \int_{B_N} f_K^p \dmu
 < \infty.
\]
Daher ist $h \in \cL^p(\Omega)$ mit
\[
 \|h\|_p
...
\]
Es gibt also ein $g \in \cL^{p'}(\Omega)$ mit
\[
 \int_\Omega h g \dmu \geq \|h\|_p -1 \geq M.
\]
Für $g_M = \sgn(f) |g|$ ist $g_M \in \cL^{p'}(\Omega)$ mit $\|g_M\|_{p'} = 1$, da $|g_M| = |g|$. Da $|f| \geq h \geq 0$ ist
\[
 \int_\Omega f g_M \dmu
 = \int_\Omega |f| |g| \dmu
...
\subsection{}
Ist $r = \infty$, so ist auch $p = q = \infty$. Für $f,g \in \cL^\infty(\Omega)$ ist dann
\[
 |f(x)g(x)| = |f(x)| |g(x)| \leq \|f\|_\infty \|g\|_\infty \text{ für $\mu$-fast alle } x \in \Omega.
\]
Das zeigt, dass $fg \in \cL^\infty(\Omega)$ und die geforderte Ungleichung gilt.

Ist $r \in [1,\infty)$, so folgt aus $f \in \cL^p(\Omega)$, dass $|f|^r \in \cL^{p/r}(\Omega)$ mit $\||f|^r\|_{p/r} = \|f\|_p^r$, und aus $g \in \cL^q(\Omega)$, dass $|g|^r \in \cL^{q/r}(\Omega)$ mit $\||g|^r\|_{q/r} = \|f\|_q^r$. Da
\[
...
\begin{ia}
 Für $k=1$ ist nichts zu zeigen, und für $k=2$ handelt es sich um den vorherigen Aufgabenteil.
\qed\end{ia}
\begin{is}
 Es sei nun $k \geq 3$ und es gelte die Aussage für $k-1$ und $2$. Wegen $\sum_{i=1}^k 1/p_i = 1/r$ ist
 \[
  \sum_{i=1}^{k-1} 1/p_i = 1/r - 1/p_k.
 \]
 
 Ist $1/r - 1/p_k = 0$, so ist $r = p_k$ und $p_1 = \ldots = p_{k-1} = \infty$. Für $1 \leq r < \infty$ ist
 \[
 ...
 \]
 also $\prod_{i=1}^k f_i \in \cL^r(\Omega)$ mit $\|\prod_{i=1}^k f_i\| \leq \prod_{i=1}^k \|f_i\|_{p_i}$.
 Für $r = \infty$ ist
 \[
  ...
  \text{ $\mu$-fast überall},
 \]
 also $\prod_{i=1}^k f_i \in \cL^\infty(\Omega) = \cL^r(\Omega)$ mit der entsprechenden Ungleichung.
 
 Ist $1/r - 1/p_k > 0$, so ist $1/r - 1/p_k \leq 1/r$ und daher
 \[
 ...
 \]
 Da $p_k, r, s \in [1,\infty]$ mit $1/p_k + 1/s = 1/r$ ist wegen $\cL^{p_k}(\Omega)$ und $\prod_{i=1}^{k-1} f_i \in \cL^s(\Omega)$ nach Induktionsvoraussetzung
 \begin{gather*}
  \prod_{i=1}^k f_i
...
\begin{align*}
     &\, \int_{\R^n \times \R^n} |f(x) g(y-x) h(y)| \dlambda_2(x,y) \\
 \leq&\, \|f\|_\infty \int_{\R^n \times \R^n} |g(y-x) h(y)| \dlambda_2(x,y) \\
 \underset{(*)}=&\, \|f\|_\infty \int_{\R^n} \int_{\R^n} |g(y-x) h(y)| \dlambda_n(x) \dlambda_n(y) \\
    =&\, \|f\|_\infty \int_{\R^n} |h(y)| \int_{\R^n} |g(y-x)| \dlambda_n(x) \dlambda_n(y) \\
    =&\, \|f\|_\infty \|g\|_1 \int_{\R^n} |h(y)| \dlambda_n(y)
    =    \|f\|_\infty \|g\|_1 \|h\|_1.
\end{align*}
Dabei wird bei $(*)$ Tonelli genutzt. Die Fälle $q = \infty$ und $r = \infty$ lassen sich analog abhandeln.

Als nächstes betrachten wir den Fall, dass $p = 1$, $q = 1$ oder $r = 1$. Ist etwa $p = 1$, so ist $1/q + 1/r = 1$ und daher
\begin{align*}
     &\, \int_{\R^n \times \R^n} |f(x) g(y-x) h(y)| \dlambda_2(x,y) \\
     \underset{(*)}=&\, \int_{\R^n} \int_{\R^n} |f(x) g(y-x) h(y)| \dlambda_n(y) \dlambda_n(x) \\
     =&\, \int_{\R^n} |f(x)| \int_{\R^n} |g(y-x) h(y)| \dlambda_n(y) \dlambda_n(x) \\
     \underset{(**)}\leq&\, \|g\|_q \|h\|_r \int_{\R^n} |f(x)| \dlambda_n(x)
     = \|f\|_1 \|g\|_q \|h\|_r.
\end{align*}
Dabei wird bei $(*)$ Tonelli und bei $(**)$ die Hölder-Ungleichung genutzt. Die Fälle $q = 1$ und $r = 1$ lassen sich analog abhandeln.
...
\begin{align*}
 ...
 \underset{(*)}=&\, \int_{\R^n} \int_{\R^n} |g(y-x)|^q |h(y)|^r \dlambda_n(x) \dlambda_n(y) \\
 =&\, \int_{\R^n} |h(y)|^r \int_{\R^n} |g(y-x)|^q \dlambda_n(x) \dlambda_n(y) \\
 =&\, \|g\|_q^q \int_{\R^n} |h(y)|^r \dlambda_n(y)
 = \|g\|_q^q \|h\|_r^r.
\end{align*}
Dabei wird bei $(*)$ überraschenderweise Tonelli genutzt.
...
Sei nun $p = \infty$. Sei zunächst $f \in \cL^\infty(\Omega)$ mit $f \geq 0$. Für $n \in \N$ definieren wir die Folge einfacher Funktionen $(f_n)_{n \geq 1}$ auf $\cL^\infty(\Omega)$ als
\[
 f_n(x) := \sup \left\{ \frac{k}{n} \leq f(x) : k \in \N \right\}.
\]
Dass $n \in \N$ $f_n \in \cL^\infty(\Omega)$ und $f_n$ einfach ist, folgt für alle $n \in \N$ direkt daraus, dass \mbox{$f \in \cL^\infty(\Omega)$}. Es ist auch klar, dass
\[
...
\[
 f_n := \sgn(f)f'_n.
\]
Dass $f_n \in \cL^\infty(\Omega)$ für alle $n \in \N$ folgt direkt daraus, dass $|f_n| = |f'_n|$ für alle $n \in \N$. Da
\[
...
\[
 \|f-h\|_p
 = \sup\left\{ \int_\Omega (f-h) g \dmu : g \in \cL^{p'}(\Omega) \text{ mit } \|g\|_{p'} \leq 1 \right\}
 = 0.
\]
