...
\begin{is}
 ...
 \]
 also $\prod_{i=1}^k f_i \in \La^r(\Omega)$ mit $\|\prod_{i=1}^k f_i\| \leq \prod_{i=1}^k \|f_i\|_{p_i}$.
 Für $r = \infty$ ist
 \[
  ...
  \text{ $\mu$-fast überall},
 \]
 also $\prod_{i=1}^k f_i \in \La^\infty(\Omega) = \La^r(\Omega)$ mit der entsprechenden Ungleichung.
 
 Ist $1/r - 1/p_k > 0$, so ist $1/r - 1/p_k \leq 1/r$ und daher
 \[
  s := \frac{1}{\frac{1}{r} - \frac{1}{p_k}} \geq r \geq 1.
 \]
 Da $\sum_{i=1}^{k-1} 1/p_i = 1/s$ ist nach der Induktionsvoraussetzung $\prod_{i=1}^{k-1} f_i \in \La^s(\Omega)$ mit
 \[
  \left\| \prod_{i=1}^{k-1} f_i \right\|_s \leq \prod_{i=1}^{k-1} \|f_i\|_{p_i}.
 \]
 Da $p_k, r, s \in [1,\infty]$ mit $1/p_k + 1/s = 1/r$ ist wegen $\La^{p_k}(\Omega)$ und $\prod_{i=1}^{k-1} f_i \in \La^s(\Omega)$ nach Induktionsvoraussetzung
 \begin{gather*}
  \prod_{i=1}^k f_i
  = \left( \prod_{i=1}^{k-1} f_i \right) f_k \in \La^r(\Omega)
 \shortintertext{mit}
...
\begin{align*}
 \|f\|_p
 &= \sup \left\{ \int_\Omega f g \dmu : g \in \La^{p'}(\Omega), \|g\|_{p'} \leq 1 \right\} \\
 &= \sup \left\{ \limesinf{k} \int_\Omega f_k g \dmu : g \in \La^{p'}(\Omega), \|g\|_{p'} \leq 1 \right\} \\
 &\leq \limesinf{k} \sup \left\{ \int_\Omega f_k g \dmu : g \in \La^{p'}(\Omega), \|g\|_{p'} \leq 1 \right\}
 = \limesinf{k} \|f_k\|_p.
\end{align*}
