...
\title{\sc Analysis III \\ \Large 1. Aufgabenblatt}
...
\subsection{}
Wie in \textbf{Aufgabe 3} gezeigt handelt es sich bei $\mc{A}_1 = \{\emptyset, \{1\}, \{2,3\}, \{1,2,3\}\}$ und $\mc{A}_2 = \{\emptyset, \{2\}, \{1,3\}, \{1,2,3\}\}$ um $\sigma$-Algebren auf $\{1,2,3\}$, während $\mc{A}_1 \cup \mc{A}_2$ keine $\sigma$-Algebra auf $\{1,2,3\}$ ist.
...
\begin{align*}
 ...
 A(p+q),
\end{align*}
also ist $A$ ein Gruppenhomomorphismus. $A$ ist injektiv, denn für $p,q \in \Q$ mit $A(p) = A(q)$ muss $\cos p = \cos q$ und $\sin p = \sin q$ gelten, also auch
\[
...
Da $A$ ein Gruppenmonomorphismus ist, und $\Q$ abzählbar unendlich, ist
\[
 G := \Img A =
 \left\{
 \begin{pmatrix}
...
Angenommen es gibt eine Abbildung $\mu$ mit den geforderten Eigenschaften. Es ist dann insbesondere $\mu(M_R) = \mu(RM) = \mu(M)$ für alle $R \in G \subseteq SO(2)$. Es folgt, dass
\begin{align*}
 1
 &= \mu(B_1(0)\setminus\{0\})
...
\subsection{}
Es sei für alle $B \subseteq \R^2$
\[
 \mu(B) =
 \begin{cases}
  1 & \text{falls } 0 \in B\\
...
Da für alle $R \in SO(2)$ die Matrix $R$ einen Vektorraumautomorphismus $\R^2 \rightarrow \R^2$ beschreibt, ist $\operatorname{Ker} R = \{0\}$, also $0 \in RA \Leftrightarrow 0 \in A$ für alle $A \subseteq \R^2$. Es ist daher $\mu(RA) = 1 \Leftrightarrow 0 \in RA \Leftrightarrow 0 \in A \Leftrightarrow \mu(A) = 1$, also $\mu(RA) = \mu(A)$ für alle $R \in SO(2)$ und $A \subseteq \R^2$ (man bemerke, dass diese Äquivalenz bereits eine Gleichheit impliziert, da $\mu$ nur zwei mögliche Werte annehmen kann).
