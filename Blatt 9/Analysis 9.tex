...
\begin{align*}
 ...
 = \frac{\sin(x)}{x}.
\end{align*}
Daher ergibt sich mit dem Satz von Fubini zum einen, dass
\begin{align*}
 \int_{[0,R] \times [0,\infty)} g \dlambda_2
 &= \int_{[0,R]} \int_{[0,\infty)} g(x,t) \dlambda(t) \dlambda(x)
 = \int_{[0,R]} \frac{\sin(x)}{x} \dlambda(x) \\
...
\begin{align*}
 &\, \int_0^R \sin(x) e^{-xt} \dx
 = \left. -\frac{1}{t} \sin(x) e^{-xt} \right|_{x=0}^5 + \frac{1}{t} \int_0^R \cos(x) e^{-xt} \dx \\
...
\[
 ...
 = R^N \alpha_N.
\]
Sei nun $N \geq 3$ beliebig aber fest. Für alle $(x_1, x_) \in \R^2$ ist
\begin{align*}
...
\end{align*}
Da die Funktion $r\sqrt{1-r^2}^{N-2}$ auf $(0,1)$ beschränkt und stetig ist, entspricht das Lebesgue-Integral dem Riemann-Integral. Dabei ergibt sich mit den Substitutionen $r = \sin t$, $\dr = \cos t \dt$ und $\cos t = x$ und $-\sin t = \dx$, dass
\begin{align*}
