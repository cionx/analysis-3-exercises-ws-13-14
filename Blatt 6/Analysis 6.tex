...
\[
 ...
 = 0.
\]
Dies zeigt die Normiertheit von $\mu$. (Zur Notation: Es ist $f \leq g$ genau dann wenn $f(x) \leq g(x)$ für alle $x \in X$.) Für $A, B \in \A$ mit $A \subseteq B$ folgt aus $\chi_A \leq \chi_B$ und $f \geq 0$, dass $f \chi_A \leq f \chi_B$ und damit
\[
 \nu(A)
 = \int_A f \dmu
 = \int f \chi_A \dmu
...
Sei nun $E \subseteq [0,1]$ messbar mit $\lambda(E) > \alpha$. Es ist $\mu(A_\delta \cap E) \leq \mu(A_\delta) < \beta$, also $\mu(E \setminus A_\delta) > \alpha - \beta$. Da $f > 0$ und $f(x) \geq \delta$ für alle $x \in E \setminus A_\delta$ ist daher
\[
 \int_E f \dlambda
